  ...
  marked.use({
    renderer: {
      code(token: any) {
        ...
        const id = `editor-cb-${++_codeBlockCounter}`;
        let highlighted = "";
        try {
          highlighted =
        ...
        // pass arbitrary code text through a JS string literal — which
        // breaks on backticks, backslashes, newlines, and quotes.
    ...
    <div class="bg-muted px-4 py-2 text-xs text-muted-foreground border-b flex justify-between items-center">
      ...
      <button data-copy-id="${id}" data-code="${dataCode}" class="hover:text-foreground transition-colors copy-btn" onclick="copyCodeToClipboard('${id}')">
        ...
        <svg class="check-icon w-4 h-4 hidden text-green-500" xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round">
