...
const LANG_LABELS: Record<string, string> = {
  cpp: "C++", java: "Java", python: "Python", javascript: "JavaScript",
};
...
type StatusCfg = {
  label: string; accentColor: string; textClass: string;
  dotClass: string; passSegClass: string; failSegClass: string; countClass: string;
};
function getStatusCfg(status: string, statusDescription?: string | null): StatusCfg {
  if (status === "accepted") return { label: "Accepted", accentColor: "#1D9E75", textClass: "text-[#1D9E75]", dotClass: "bg-[#1D9E75]", passSegClass: "bg-[#1D9E75]", failSegClass: "bg-[#E24B4A]", countClass: "text-[#1D9E75]" };
  if (status === "time_limit_exceeded") return { label: "Time Limit Exceeded", accentColor: "#BA7517", textClass: "text-[#BA7517]", dotClass: "bg-[#BA7517]", passSegClass: "bg-[#1D9E75]", failSegClass: "bg-[#BA7517]", countClass: "text-[#BA7517]" };
  if (status === "wrong_answer") return { label: "Wrong Answer", accentColor: "#E24B4A", textClass: "text-[#E24B4A]", dotClass: "bg-[#E24B4A]", passSegClass: "bg-[#1D9E75]", failSegClass: "bg-[#E24B4A]", countClass: "text-[#E24B4A]" };
  if (status === "compilation_error") return { label: "Compilation Error", accentColor: "#E24B4A", textClass: "text-[#E24B4A]", dotClass: "bg-[#E24B4A]", passSegClass: "bg-[#1D9E75]", failSegClass: "bg-[#E24B4A]", countClass: "text-[#E24B4A]" };
  return { label: "Runtime Error", accentColor: "#E24B4A", textClass: "text-[#E24B4A]", dotClass: "bg-[#E24B4A]", passSegClass: "bg-[#1D9E75]", failSegClass: "bg-[#E24B4A]", countClass: "text-[#E24B4A]" };
}

// ─── Atoms ────────────────────────────────────────────────────────────────────

function SectionLabel({ children }: { children: React.ReactNode }) {
  return <p className="text-[10px] font-semibold tracking-widest uppercase text-muted-foreground mb-2">{children}</p>;
}
function PlainCodeBlock({ children, variant = "default" }: { children: React.ReactNode; variant?: "default" | "error" | "wrong" }) {
  const isRed = variant === "error" || variant === "wrong";
  return (
    <pre className={["rounded-lg px-3 py-2.5 text-xs font-mono leading-relaxed whitespace-pre-wrap break-words overflow-x-auto",
      isRed ? "bg-destructive/5 border border-destructive/20 text-destructive" : "bg-secondary border border-border/10 text-foreground"].join(" ")}>
      {children}
    </pre>
  );
}
...
function Divider() { return <div className="w-full h-px bg-border/10" />; }

// ─── Progress bar ─────────────────────────────────────────────────────────────

function ProgressBar({ passed, total, cfg }: { passed: number; total: number; cfg: StatusCfg }) {
  return (
    <div className="flex flex-col gap-2">
      <div className="flex items-baseline justify-between">
        <span className="text-xs text-muted-foreground">Test cases passed</span>
        <span className={`text-[13px] font-semibold tabular-nums ${cfg.countClass}`}>{passed} / {total}</span>
      </div>
      <div className="flex gap-1">
        {Array.from({ length: total }).map((_, i) => {
          let segClass = i < passed ? cfg.passSegClass : i === passed && passed < total ? cfg.failSegClass : "bg-secondary";
          return <div key={i} className={`h-[5px] flex-1 rounded-full transition-colors duration-300 ${segClass}`} />;
        })}
      </div>
    </div>
  );
...
function StatsFooter({ runtime, memory, language, passRate, avgRuntime, submittedAt }: {
  runtime: number | null; memory: number | null; language: string;
  passRate?: string; avgRuntime?: number; submittedAt?: string;
}) {
  const items: { label: string; value: string }[] = [];
  if (runtime != null) items.push({ label: "Runtime", value: runtime + "ms" });
  if (memory != null) { const v = formatMemoryMB(memory); if (v) items.push({ label: "Memory", value: v }); }
  if (language) items.push({ label: "Language", value: LANG_LABELS[language] ?? language });
  if (passRate != null) items.push({ label: "Pass Rate", value: passRate + "%" });
  if (avgRuntime != null) items.push({ label: "Avg Runtime", value: avgRuntime + "ms" });
  if (submittedAt) items.push({ label: "Submitted", value: formatTimestamp(submittedAt) });
...
function TestCaseTable({ testCaseResults }: { testCaseResults: TestCaseResult[] }) {
        ...
        {testCaseResults.map((tc, i) => {
          ...
          return (
            <div key={i} className="border-b border-border/10 last:border-b-0">
              ...
              >
                {/* Status icon */}
                <span className={`flex-shrink-0 w-5 h-5 rounded-full flex items-center justify-center text-[10px] font-bold ${isPass ? "bg-[#1D9E75]/15 text-[#1D9E75]" : "bg-destructive/15 text-destructive"}`}>
                  {isPass ? "✓" : "✗"}
                </span>
                ...
                {/* Status label */}
                <span className={`text-xs flex-1 ${isPass ? "text-[#1D9E75]" : "text-destructive"}`}>
                ...
                <svg
                  ...
                  stroke="currentColor" strokeWidth="2" className="flex-shrink-0 text-muted-foreground transition-transform duration-200"
                ...
                <div className="px-3 pb-3 pt-1 flex flex-col gap-2.5 bg-secondary/20">
                  ...
                  {tc.actualOutput != null && (
                    <div>
                      ...
                      <PlainCodeBlock variant={isPass ? "default" : "wrong"}>{tc.actualOutput}</PlainCodeBlock>
                    </div>
                  )}
                </div>
              )}
...
const CustomTooltip = ({ active, payload, label }: any) => {
  ...
  return (
    <div className="rounded-lg border border-border/20 bg-card px-3 py-2 text-xs shadow-lg">
      ...
      <p className="text-muted-foreground">{payload[0].value} submission{payload[0].value !== 1 ? "s" : ""}</p>
    </div>
  );
};

function RuntimeDistributionChart({
  distribution, percentile, language,
}: {
  distribution: RuntimeBucket[]; percentile: number; language: string;
}) {
    ...
    <div className="flex flex-col gap-3">
      <div className="flex items-center justify-between">
        ...
        <span className="text-xs text-[#1D9E75] font-medium">
          Faster than {percentile}% of {LANG_LABELS[language] ?? language} submissions
        </span>
      </div>
      <div className="h-32">
        <ResponsiveContainer width="100%" height="100%">
          <BarChart data={distribution} margin={{ top: 4, right: 4, left: -28, bottom: 0 }} barCategoryGap="20%">
            <XAxis
              dataKey="bucketLabel"
            ...
            <Bar dataKey="count" radius={[3, 3, 0, 0]}>
              {distribution.map((entry, index) => (
                <Cell
                  key={index}
                  fill={entry.isUserBucket ? "#1D9E75" : "hsl(var(--muted-foreground) / 0.3)"}
                />
              ))}
            </Bar>
          </BarChart>
      ...
      <p className="text-[11px] text-muted-foreground">
        Green bar = your runtime. Each bar represents a range of submission runtimes.
      </p>
    </div>
  );
}
...
function EmptyState() {
  return (
    <div className="h-full w-full flex flex-col items-center justify-center gap-3 py-14 text-muted-foreground select-none">
      <svg width="34" height="34" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.4" className="opacity-25">
...
function SubmitResultCard({ result, httpStatus }: { result: SubmitCodeSuccess | SubmitCodeError; httpStatus: number }) {
  ...
  return (
    <div className="rounded-xl border border-border/20 overflow-hidden bg-card animate-in fade-in slide-in-from-bottom-1 duration-200">
      ...
      >
        <div className="flex flex-col gap-1">
          <div className="flex items-center gap-2">
            ...
            <span className={`text-[18px] font-semibold tracking-tight ${cfg.textClass}`}>{cfg.label}</span>
          </div>
      ...
      <div className="px-5 py-4 flex flex-col gap-4">
        ...
        {/* Success runtime chips */}
        {isSuccess && (
          <div className="flex gap-2 flex-wrap">
            <InfoChip label="Runtime" value={success.runtimeInMilliseconds + "ms"} />
            <InfoChip label="Memory" value={formatMemoryMB(success.memoryInMegabytes) ?? ""} />
          </div>
        )}

        {/* Runtime distribution chart — Accepted only */}
        {hasDistribution && (
          <>
            <Divider />
        ...
        {/* First failed test case I/O — shown above the table as the highlighted failure */}
        {showIO && (
          <>
            <Divider />
            <div className="flex flex-col gap-2.5">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-2.5">
                {ft!.input && (
                  <div><SectionLabel>Input</SectionLabel><PlainCodeBlock>{ft!.input}</PlainCodeBlock></div>
                )}
                {ft!.expectedOutput && (
                  <div><SectionLabel>Expected Output</SectionLabel><PlainCodeBlock>{ft!.expectedOutput}</PlainCodeBlock></div>
                )}
                {showActualOutput && (
        ...
        <div>
          ...
          <div className="rounded-lg overflow-hidden border border-border/10">
            <SyntaxHighlighter
              ...
              style={atomOneDark}
              showLineNumbers
              customStyle={{ margin: 0, padding: "12px", fontSize: "12px", lineHeight: "1.65", background: "transparent", maxHeight: "420px", overflowY: "auto" }}
              lineNumberStyle={{ minWidth: "2.5em", paddingRight: "1em", color: "#4b5563", userSelect: "none" }}
            >
              {code}
            </SyntaxHighlighter>
          </div>
