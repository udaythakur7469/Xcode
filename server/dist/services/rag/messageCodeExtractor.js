export const extractCodeFromMessage = (message) => {
    ...
    const looksLikeCode = (message.includes("function") ||
...
const containsErrorLog = (text) => {
    return /exception|stack trace|traceback|error at|segmentation fault|runtime error/i.test(text);
};
const containsTestCase = (text) => {
    return /test case|failing case|input:\s*\n|output:\s*\n/i.test(text);
};
const containsIO = (text) => {
...
export const extractArtifacts = ({ userMessage, providedCode, providedError, providedTestCase, providedIO, }) => {
    const detectedArtifacts = [];
    const hasCode = Boolean(providedCode?.trim()) ||
        Boolean(extractCodeFromMessage(userMessage));
    const hasErrorLog = Boolean(providedError?.trim()) || containsErrorLog(userMessage);
    const hasTestCase = Boolean(providedTestCase?.trim()) || containsTestCase(userMessage);
    const hasIO = Boolean(providedIO?.trim()) || containsIO(userMessage);
    if (hasCode)
        detectedArtifacts.push("CODE");
    if (hasErrorLog)
        detectedArtifacts.push("ERROR_LOG");
    if (hasTestCase)
        detectedArtifacts.push("TEST_CASE");
    if (hasIO)
        detectedArtifacts.push("INPUT_OUTPUT");
...
const computeExtractionConfidence = ({ hasCode, hasErrorLog, hasTestCase, hasIO, userMessage, }) => {
    let score = 0;
    ...
    if (/\b(above|earlier|previous|my code|my solution|already shared)\b/i.test(userMessage)) {
        score -= 0.25;
    }
