...
import { extractArtifacts, extractCodeFromMessage, } from "./messageCodeExtractor.js";
...
const ruleBasedCodeDetection = (message) => {
...
const ruleBasedErrorDetection = (message) => {
    const normalizedMessage = message.toLowerCase().trim();
...
const ruleBasedTestCaseDetection = (message) => {
    const normalizedMessage = message.toLowerCase().trim();
...
const llmBasedArtifactDetection = async ({ userMessage, normalizedQuery, intent, ifCodePresent, ifTestCasePresent, ifErrorPresent, ifIOPresent, }) => {
    try {
...
export const missingArtifactsDetector = async ({ userMessage, normalizedQuery, detectedIntent, providedCode, providedError, providedTestCase, providedIO, }) => {
    ...
    console.log("[missingArtifactsDetector] Code quality evaluation:", codeQuality);
    ...
    const shouldInvokeLLM = !needsCode &&
        codeRequiringIntents.includes(detectedIntent) &&
        !artifacts.hasCode;
    if (shouldInvokeLLM) {
        console.log("[missingArtifactsDetector] Rule-based inconclusive, invoking LLM");
    ...
    const blockGeneration = missingArtifacts.length > 0 || artifacts.extractionConfidence < 0.5;
    const result = {
...
const generateRequestMessage = (artifacts, confidence) => {
