...
import { RefusalLevel, SolutionPermissionMode, } from "./types.js";
...
const computeRegeneratePenaltyScore = (regenerateCount) => {
...
const applyPermissionCap = (rawScore, permissionMode, refusalLevel) => {
...
export const answerConfidenceScorer = async (input) => {
    const { retrievalConfidence, intentConfidence, artifactConfidence, topicSimilarityScore, permissions, regenerateCount, problemTitle, } = input;
    ...
    const rawWeightedScore = retrievalComponent +
        intentComponent +
        artifactComponent +
        topicComponent +
        regenerateComponent;
    const clampedScore = clamp(rawWeightedScore, 0, 1);
    const cappedScore = applyPermissionCap(clampedScore, permissions.solutionPermissionMode, permissions.refusalLevel);
