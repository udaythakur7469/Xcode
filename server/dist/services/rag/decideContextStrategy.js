...
export const decideContextStrategy = (input) => {
    const { topicShiftStrategy, hasLongRangeReference, longRangeConfidence, regenerateCount, aiModelChanged, } = input;
    // ── TARGETED_BACKTRACK ────────────────────────────────────────────
    // Long-range reference detected with sufficient confidence.
    // Load recent messages AND trigger semantic history search.
    if (hasLongRangeReference &&
    ...
    if (regenerateCount > 0 || aiModelChanged) {
        const expandedCount = Math.min(EXPANDED_BASE_COUNT + regenerateCount * EXPANDED_INCREMENT, EXPANDED_MAX_COUNT);
        return {
            strategy: "EXPANDED",
