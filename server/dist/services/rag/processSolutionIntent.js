...
import { generateRefusalMessage, getOrCreateHelpState, incrementSolutionAttempt, resolvePermissions, updateHintLevel, updateRefusalLevel, } from "./permissionResolver.js";
...
export const solutionIntentProcessor = async (userId, problemTitle, userMessage, previousUserMessages) => {
    ...
    if (!problemTitle || !userId) {
        console.log("[solutionIntentGate] No problem context, allowing with hints only");
        return {
            permissions: {
    ...
    if (intent !== SolutionIntent.FULL_SOLUTION) {
        ...
        const helpState = await getOrCreateHelpState(userId, problemContext.problemId);
        const permissions = resolvePermissions(intent, problemContext.userSolved, helpState.solutionAttempts, helpState.maxHintLevel, confidence);
        console.log("[solutionIntentProcessor] Non-solution request, allowing:", permissions);
        return {
            permissions,
            shouldBlock: false,
            intent,
    ...
    console.log("[solutionIntentProcessor] Full solution requested, user solved:", hasSolved);
    if (hasSolved) {
        // User solved it - allow full solution
        const permissions = resolvePermissions(intent, true, 0, HintLevel.NONE, confidence);
        console.log("[solutionIntentProcessor] User solved, allowing full solution");
        return {
            permissions,
            shouldBlock: false,
            intent,
    ...
    console.log("[solutionIntentProcessor] User hasn't solved, blocking solution request");
    // Increment solution attempts
    const attemptCount = await incrementSolutionAttempt(userId, problemContext.problemId);
    console.log("[solutionIntentProcessor] Solution attempt count:", attemptCount);
    ...
    const permissions = resolvePermissions(intent, false, attemptCount, newHintLevel, confidence);
    await updateRefusalLevel(userId, problemContext.problemId, permissions.refusalLevel);
    console.log("[solutionIntentProcessor] Resolved permissions:", permissions);
    // Generate refusal message
    const blockMessage = generateRefusalMessage(permissions.refusalLevel, attemptCount, problemContext.problemTitle);
    return {
        permissions,
        shouldBlock: true,
        blockMessage,
