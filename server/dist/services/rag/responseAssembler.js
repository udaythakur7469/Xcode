...
import { HintLevel, RefusalLevel, SolutionPermissionMode, } from "./types.js";
...
const HINT_FORMAT_PREFIX_MAP = {
    [HintLevel.NONE]: "",
    [HintLevel.CONCEPT]: "💡 Conceptual hint: ",
    [HintLevel.DATA_STRUCTURE]: "🔧 Structural hint: ",
    [HintLevel.EDGE_CASES]: "⚠️ Edge case hint: ",
};
const GREETING_PREFIX_PATTERN = /^(hi|hello|hey|howdy|greetings|good\s+(morning|afternoon|evening|day))/i;
const applyRefusalPrefix = (rawAnswer, permissionMode, refusalLevel) => {
    if (permissionMode !== SolutionPermissionMode.DENY_FULL_SOLUTION) {
        return rawAnswer;
    }
    return REFUSAL_PREFIX_MAP[refusalLevel] + rawAnswer;
};
const applyHintFormatting = (rawAnswer, permissionMode, maxHintLevel) => {
...
const appendConfidenceNotice = (answer, overallConfidence, injectMetadata) => {
    if (!injectMetadata) {
        return answer;
    }
...
const mergePrefixSegment = (prefixSegment, formattedAnswer) => {
...
const normalizeOutputWhitespace = (response) => response.replace(/\n{3,}/g, "\n\n").trim();
export const responseAssembler = async (input) => {
    const { rawAnswer, prefixSegment, permissions, overallConfidence, injectConfidenceMetadata, generationSucceeded, problemTitle, } = input;
    ...
    assembled = applyRefusalPrefix(assembled, permissions.solutionPermissionMode, permissions.refusalLevel);
    assembled = applyHintFormatting(assembled, permissions.solutionPermissionMode, permissions.maxHintLevel);
    assembled = appendConfidenceNotice(assembled, overallConfidence, injectConfidenceMetadata);
