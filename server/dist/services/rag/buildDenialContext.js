...
import { computeNextStyleState, mapIntensityToStyle, STYLE_COOLDOWN_MS, } from "./styleDecayEngine.js";
import { getRefusalState, upsertRefusalState, } from "./refusalStateRepository.js";
// ============================================================
// buildDenialContext
//
...
// ============================================================
export const buildDenialContext = async (input) => {
    const { userId, problemId, denialReason, refusalLevel, solutionAttemptCount, intentConfidence, detectedIntent, userSolved, isNewTopic, missingArtifacts, } = input;
    ...
    const cooldownExpired = previousState !== null &&
    ...
    const refusalStyle = mapIntensityToStyle(nextState.intensity);
    ...
    upsertRefusalState(userId, problemId, nextState).catch((err) => logger.error("buildDenialContext: failed to persist refusal state", {
        err,
        userId,
        problemId,
    }));
