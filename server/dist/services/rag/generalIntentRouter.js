...
function keywordFastPath(message) {
    ...
    if (GREETING_PATTERNS.some((p) => p.test(lower)))
        return "GREETING";
    if (SUBMISSION_KEYWORDS.some((k) => lower.includes(k)))
        return "MY_SUBMISSIONS";
    if (STATS_KEYWORDS.some((k) => lower.includes(k)))
        return "MY_STATS";
    if (STICKY_KEYWORDS.some((k) => lower.includes(k)))
        return "MY_STICKY_NOTES";
    if (POST_KEYWORDS.some((k) => lower.includes(k)))
        return "MY_POSTS";
    if (COMMENT_KEYWORDS.some((k) => lower.includes(k)))
        return "MY_COMMENTS";
    if (INTERVIEW_HISTORY_KEYWORDS.some((k) => lower.includes(k)))
        return "MY_INTERVIEW_HISTORY";
    if (PLATFORM_KEYWORDS.some((k) => lower.includes(k)))
        return "PLATFORM_FEATURE";
    if (PROBLEM_POSTS_KEYWORDS.some((k) => lower.includes(k)))
        return "PROBLEM_POSTS";
    if (TEST_CASE_KEYWORDS.some((k) => lower.includes(k)))
        return "TEST_CASES";
    if (PROBLEM_STATS_KEYWORDS.some((k) => lower.includes(k)))
    ...
    if (isShort && GENERAL_CS_KEYWORDS.some((k) => lower.startsWith(k)))
        return "GENERAL_CS";
    return null;
}
// DB lookup map — which intents need a DB call
const DB_REQUIRED_INTENTS = new Set([
    "MY_SUBMISSIONS",
    "MY_STATS",
    "MY_STICKY_NOTES",
    "MY_POSTS",
...
// ─── Main detector ────────────────────────────────────────────────────────────
export async function detectGeneralIntent(userMessage) {
    const notGeneral = {
...
extractedKeyword: fill this for sticky note / post searches — the topic the user is searching for.`,
        ...
        const intent = parsed.intent ?? "NOT_GENERAL";
        return {
