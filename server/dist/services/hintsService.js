...
export const hashCode = (normalized) => {
    return crypto.createHash("sha256").update(normalized).digest("hex");
};
export const isLowEffort = (normalized) => {
    return normalized.replace(/\s/g, "").length < 60;
};
export const hasCodeChangedSignificantly = (storedHash, currentHash) => {
    return storedHash !== currentHash;
};
export const buildPrompt = (problemTitle, problemDescription, userCode, language, lowEffort) => {
