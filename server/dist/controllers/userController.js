...
export const authenticatedUser = async (req, res) => {
    try {
        ...
        // Process languages
        const allLanguages = solvedProblems.flatMap((sp) => sp.user.submissions.map((sub) => sub.language));
        ...
        const difficultyCounts = solvedProblems.reduce((acc, sp) => {
            acc.totalSolved++;
        ...
        res.status(200).json({
            ...
            user: {
                ...
                stats: {
                    ...
                    languages: Object.entries(languageFrequency).map(([language, count]) => ({
                        language,
                        count,
                    })),
