...
// ─────────────────────────────────────────────────────────────────────────────
function computeTalkRatio(transcript) {
...
`,
            system: "You are a professional interviewer analyzing a mock interview. Evaluate objectively, surfacing both strengths and genuine weaknesses with specific evidence from the transcript.",
        });
        const numericId = Number(id);
        const feedback = await prisma.feedback.create({
            data: {
                ...
                categoryScores: {
                    create: object.categoryScores.map((c) => ({
                        name: c.name,
                        score: c.score,
                        comment: c.comment,
                    })),
                ...
                questionScores: {
                    create: object.questionScores.map((q) => ({
                ...
                keyMoments: {
                    create: object.keyMoments.map((m) => ({
                        type: m.type,
                ...
                recommendedTopics: {
                    create: object.recommendedTopics.map((t) => ({
                        topic: t.topic,
                        reason: t.reason,
                        priority: t.priority,
                        tags: t.tags,
