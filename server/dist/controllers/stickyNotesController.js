...
export const deleteStickyNote = async (req, res, next) => {
    try {
        ...
        deleteStickyNoteEmbedding(id).catch((err) => logger.error("sticky embed delete failed", err));
        return res.status(200).json({
...
export const bulkCreateStickyNotes = async (req, res, next) => {
    try {
        ...
        const createdNotes = await prisma.$transaction(notes.map((note) => prisma.stickyNote.create({
            data: {
                userId,
