 ...
 */
async function walkToLatestLeaf(startId, chatId, visited = new Set()) {
    if (visited.has(startId))
        return [startId]; // cycle guard
    visited.add(startId);
 ...
 */
async function resolveActivePath(chatId, persistedPath) {
...
export const createBranch = async (req, res, next) => {
    ...
    sourceMessageId, message, // New text (for edit: new user text; for regenerate: same user text)
    branchType, // "edit" | "regenerate"
    aiModel, problemTitle, } = req.body;
    const userId = req.user?.Id || req.user?.userId;
    try {
        if (!chatId || !sourceMessageId || !message || !branchType || !aiModel) {
            throw createHttpError.BadRequest("Missing required fields: chatId, sourceMessageId, message, branchType, aiModel");
        }
        if (branchType !== "edit" && branchType !== "regenerate") {
            throw createHttpError.BadRequest("branchType must be 'edit' or 'regenerate'");
        }
        const chat = await prisma.chat.findUnique({ where: { id: chatId } });
        if (!chat || chat.userId !== userId) {
            throw createHttpError.NotFound("Chat not found");
        ...
        if (sourceMessage.ChatId !== chatId) {
            throw createHttpError.BadRequest("Source message does not belong to this chat");
        }
        ...
        if (branchType === "edit") {
            ...
            if (sourceMessage.role !== "user") {
                throw createHttpError.BadRequest("Edit branches must target a user message");
            }
            ...
            const pathUpToParent = buildPathUpToParent(chat.activePath, sourceMessageId);
            ...
            if (problemTitle) {
                ...
                if (prob) {
                    resetRefusalStateForBranch(userId, prob.id, sourceMessage.parentId).catch((err) => logger.error("createBranch: resetRefusalStateForBranch failed", {
                        err,
                    }));
                }
            }
            // Inherit parent branch's conversation summary into new branch leaf
            inheritSummaryForBranch(chatId, sourceMessage.parentId, newUserMessage.id).catch((err) => logger.error("createBranch: inheritSummaryForBranch failed", { err }));
            ...
            if (sourceMessage.role !== "user") {
                throw createHttpError.BadRequest("Regenerate branches must target a user message");
            }
            ...
            const pathUpToSource = buildPathUpToSource(chat.activePath, sourceMessageId);
            ...
            if (problemTitle) {
                ...
                if (prob) {
                    resetRefusalStateForBranch(userId, prob.id, sourceMessageId).catch((err) => logger.error("createBranch: resetRefusalStateForBranch failed", {
                        err,
                    }));
                }
            }
            // Inherit parent branch's summary into new branch leaf
            inheritSummaryForBranch(chatId, sourceMessageId, aiPlaceholder.id).catch((err) => logger.error("createBranch: inheritSummaryForBranch failed", { err }));
            res.status(200).json(responsePayload);
            generateAsync({
                aiPlaceholderId: aiPlaceholder.id,
                chatId,
...
export const updateActivePath = async (req, res, next) => {
    ...
    try {
        if (!chatId || !Array.isArray(activePath)) {
            throw createHttpError.BadRequest("Please provide chatId and activePath array");
        }
        const chat = await prisma.chat.findUnique({ where: { id: chatId } });
        if (!chat || chat.userId !== userId) {
            throw createHttpError.NotFound("Chat not found");
...
export const getMessages = async (req, res, next) => {
    ...
    try {
            ...
            if (msg.status === "thinking" && msg.role === "assistant") {
                ...
                if (timeSince > THINKING_TIMEOUT_MS) {
                    prisma.message
                        ...
                        .catch((err) => logger.error("Failed to update stale message:", err));
                    return {
                        id: msg.id,
        ...
        if (activePath.join(",") !== chat.activePath.join(",") &&
            activePath.length > 0) {
            prisma.chat
...
export const updateFeedback = async (req, res, next) => {
    ...
    try {
        ...
        if (feedback !== "LIKE" && feedback !== "DISLIKE" && feedback !== null) {
            throw createHttpError.BadRequest("feedback must be 'LIKE', 'DISLIKE', or null");
        }
        const message = await prisma.message.findUnique({
            where: { id: messageId },
            include: { Chat: { select: { userId: true } } },
        ...
        if (message.role !== "assistant") {
            throw createHttpError.BadRequest("Feedback can only be set on AI messages");
        }
 ...
 */
function buildPathUpToParent(activePath, targetId) {
 ...
 */
function buildPathUpToSource(activePath, sourceId) {
 ...
 */
function generateAsync({ aiPlaceholderId, chatId, userMessageId, currentUserMessage, aiModel, lastMessageModel, userId, problemTitle, problemId, userSolved, activePath, isBranch, }) {
    (async () => {
        try {
            ...
            if (!preCheck || !preCheck.Chat) {
                logger.info(`Message ${aiPlaceholderId} or its chat was deleted before generation`);
                return;
            }
            ...
            if (!postCheck || !postCheck.Chat) {
                logger.info(`Message ${aiPlaceholderId} or its chat was deleted during generation`);
                return;
            }
...
export const sendChatEmail = async (req, res, next) => {
    const { recipientEmail, shareUrl, chatTitle } = req.body;
