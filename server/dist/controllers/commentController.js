    ...
    try {
        ...
        if (parentId) {
            ...
            if (parentComment.postId !== Number(postId)) {
                throw createHttpError.BadRequest("Parent comment does not belong to this post");
            }
        }
        ...
        try {
            if (req.cache) {
                ...
                if (parentId)
                    tags.push(`comment:${Number(parentId)}:replies`);
...
export const reactToComment = async (req, res, next) => {
    try {
        ...
        if (!action || (action !== "like" && action !== "dislike")) {
            throw createHttpError.BadRequest("action must be either 'like' or 'dislike'");
        }
        ...
        if (!existingReaction) {
            ...
            const newLikes = existingReaction.type === "like" ? currentLikes - 1 : currentLikes + 1;
            const newDislikes = existingReaction.type === "dislike"
                ? currentDislikes - 1
                : currentDislikes + 1;
            try {
                if (req.cache) {
...
export const deleteComment = async (req, res, next) => {
    try {
        ...
        if (comment.authorId !== userId) {
            throw createHttpError.Forbidden("You are not authorized to delete this comment");
        }
...
export const editComment = async (req, res, next) => {
    try {
        ...
        if (comment.authorId !== userId) {
            throw createHttpError.Forbidden("You are not authorized to edit this comment");
        }
