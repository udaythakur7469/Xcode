 ...
 * Per-user cache key.
 */
router.route("/checkUser").get(authMiddleware, userReadLimiter, cacheMiddleware(redis, {
    ttl: 300, // 5 minutes
    autoCache: {
        tags: ["user:profile"],
        includeAuth: true,
 ...
 */
router.route("/userLanguages").get(authMiddleware, userReadLimiter, cacheMiddleware(redis, {
    ttl: 300,
    autoCache: {
        tags: ["user:languages"],
        includeAuth: true,
 ...
 */
router.route("/heatmap").get(authMiddleware, userReadLimiter, cacheMiddleware(redis, {
    ttl: 300,
    autoCache: {
        tags: ["user:heatmap"],
        includeAuth: true,
