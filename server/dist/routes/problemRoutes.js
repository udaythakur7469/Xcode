 ...
 */
router.route("/searchProblems").get(readLimiter, cacheMiddleware(redis, {
    ttl: 300,
    autoCache: {
        tags: ["problems:search"],
        keyGenerator: (req) => `problems:search:${req.query.query}`,
    },
}), searchProblems);
/**
...
router.route("/problemDetail").get(optionalAuthMiddleware, readLimiter, cacheMiddleware(redis, {
    ...
    autoCache: {
        ...
        keyGenerator: (req) => `problem:detail:${req.query.title}`,
    },
}), getProblemByTitle);
/**
...
router.route("/getEditorials").get(optionalAuthMiddleware, readLimiter, cacheMiddleware(redis, {
    ...
    autoCache: {
        ...
        keyGenerator: (req) => `editorial:${req.query.title}`,
    },
...
router.route("/getTestCases").get(optionalAuthMiddleware, readLimiter, cacheMiddleware(redis, {
    ...
    autoCache: {
        ...
        keyGenerator: (req) => `problem:testcases:${req.query.title}`,
    },
}), getTestCases);
/**
