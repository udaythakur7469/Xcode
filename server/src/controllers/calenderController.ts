...
export const getActivityData = async (req: any, res: any, next: any) => {
  ...
  try {
    ...
    const activityMap: Record<
      string,
      { solvedCount: number; hasPotdSolved: boolean; hasRevisionDue: boolean }
    > = {};
...
export const getDayStats = async (req: any, res: any, next: any) => {
  const userId = req.user?.userId ?? req.user?.id;
  if (!userId) return next(createHttpError.Unauthorized());
  ...
  if (!date || typeof date !== "string")
    return next(
      createHttpError.BadRequest("date query param is required (YYYY-MM-DD)"),
    );
  ...
  try {
    ...
    try {
      ...
      codingTime = Math.round((runtimes._sum.runtime ?? 0) / 60_000);
    } catch {}
...
export const getRangeStats = async (req: any, res: any, next: any) => {
  const userId = req.user?.userId ?? req.user?.id;
  if (!userId) return next(createHttpError.Unauthorized());
  ...
  if (!from || !to)
    return next(
      createHttpError.BadRequest("from and to query params are required"),
    );

  try {
    const rangeStart = parseIso(from as string);
    const rangeEnd = addDays(parseIso(to as string), 1); // inclusive end
...
export const getPotd = async (req: any, res: any, next: any) => {
  ...
  try {
    ...
    const weights: Record<string, number> = { easy: 1, medium: 3, hard: 2 };
    const totalWeight = candidates.reduce(
      (sum, p) => sum + (weights[p.difficulty] ?? 1),
      0,
    );
...
export const getRevisionQueue = async (req: any, res: any, next: any) => {
  const userId = req.user?.userId ?? req.user?.id;
  if (!userId) return next(createHttpError.Unauthorized());
  ...
  try {
    ...
    const result = deduplicated.map((r) => {
      ...
      if (diffDays === 0) dueDateStr = "Due today";
      else if (diffDays === 1) dueDateStr = "Due yesterday";
      else if (diffDays > 1) dueDateStr = `Due ${diffDays} days ago`;
      else if (diffDays === -1) dueDateStr = "Due tomorrow";
      else dueDateStr = `Due in ${Math.abs(diffDays)} days`;
