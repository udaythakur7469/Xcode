...
export const updateProfilePicture = async (req, res) => {
  try {
    ...
    try {
      ...
      console.error(
        "Cache invalidation error in updateProfilePicture",
        cacheErr,
      );
    }
...
export const updateProfile = async (req, res) => {
  try {
    ...
    if (institution !== undefined) scalarUpdate.institution = institution;
    ...
    try {
      if (req.cache) await req.cache.invalidateByTags(["user:profile"]);
    } catch (cacheErr) {
