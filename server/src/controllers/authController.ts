...
const issueTokens = async (res: Response, userId: number): Promise<void> => {
...
export const sendMagicLink = async (
  ...
  try {
    ...
    await sendEmail(
      email,
      "Your Xcode sign-in link",
      magicLinkEmail({ magicLinkUrl, email }),
    );
...
export const forgotPassword = async (
  ...
  try {
    ...
    try {
      ...
      await sendEmail(
        email,
        "Reset your Xcode password",
        forgotPasswordEmail({ resetUrl, name: user.name }),
      );
    } catch (serviceError: any) {
      // Silently swallow NO_USER — prevents email enumeration attacks
      if (serviceError?.status !== 404) throw serviceError;
    }
...
export const resetPassword = async (
  ...
  try {
    ...
    sendEmail(
      user.email,
      "Your Xcode password has been changed",
      passwordResetSuccessEmail({ name: user.name }),
    ).catch((err) =>
