  ...
  try {
    ...
    const { object } = await generateObject({
      ...
      prompt: `
        ...
        `,
      system:
        "You are a professional interviewer analyzing a mock interview. Your task is to evaluate the candidate based on structured categories and provide a final verdict according to the scoring guidelines.",
    });
    ...
    const feedback = await prisma.feedback.create({
      data: {
        ...
        categoryScores: {
          create: object.categoryScores.map((category: any) => ({
            name: category.name,
