 ...
 */
router.route("/getBasePostTemplate").get(
...
router.route("/getPosts").get(
  ...
  cacheMiddleware(redis, {
    ttl: 60,
    autoCache: {
      tags: (req: any) => ["posts", `posts:problem:${req.query.problemTitle}`],
      includeAuth: true,
      keyGenerator: (req: any) => {
        const userId = req.user?.userId || "guest";
...
router.route("/getDraftPosts").get(
  authMiddleware,
  userReadLimiter,
  cacheMiddleware(redis, {
    ttl: 60,
...
router.route("/getDraftPostById").get(
  authMiddleware,
  userReadLimiter,
  cacheMiddleware(redis, {
    ttl: 60,
 ...
 */
router.route("/getPostTags").get(
  readLimiter,
  cacheMiddleware(redis, {
    ttl: 600,
    autoCache: {
      tags: (req: any) => ["post:tags", `post:tags:${req.query.problemTitle}`],
      keyGenerator: (req: any) => `post:tags:${req.query.problemTitle}`,
    },
  }),
  getCombinedTags,
);
 ...
 */
router.route("/getPostReactions").get(
  authMiddleware,
  readLimiter,
  cacheMiddleware(redis, {
    ttl: 30,
    autoCache: {
      tags: (req: any) => [`post:${req.query.postId}:reactions`],
      includeAuth: true,
      keyGenerator: (req: any) => {
        const userId = req.user?.userId;
 ...
 */
router.route("/getPostDataById").get(
  optionalAuthMiddleware,
  readLimiter,
  cacheMiddleware(redis, {
    ttl: 300,
    autoCache: {
      tags: (req: any) => ["posts", `post:${req.query.id}`],
      includeAuth: true,
      keyGenerator: (req: any) => {
        const userId = req.user?.userId || "guest";
