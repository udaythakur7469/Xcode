  ...
  cacheMiddleware(redis, {
    ...
    autoCache: {
      ...
      keyGenerator: (req: any) =>
        `calendar:activity:user:${req.user?.userId ?? req.user?.id}`,
    },
  }),
  getActivityData,
);
...
router.get(
  ...
  cacheMiddleware(redis, {
    ...
    autoCache: {
      ...
      includeAuth: true,
      keyGenerator: (req: any) =>
        `calendar:dayStats:user:${req.user?.userId ?? req.user?.id}:date:${req.query.date}`,
    },
  }),
  getDayStats,
);
...
router.get(
  ...
  cacheMiddleware(redis, {
    ...
    autoCache: {
      ...
      includeAuth: true,
      keyGenerator: (req: any) =>
        `calendar:rangeStats:user:${req.user?.userId ?? req.user?.id}:from:${req.query.from}:to:${req.query.to}`,
    },
  }),
  getRangeStats,
);
