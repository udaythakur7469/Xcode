...
interface MemoryLimitExceededResult extends BaseProcessedResult {
  success: false;
 ...
 */
export const processSubmissionResult = (result, errorInfo, language): ProcessedResult => {
 ...
 */
export const processRuntimeError = (
  result,
  statusId,
  language?: string,
): RuntimeErrorResult | MemoryLimitExceededResult => {
  ...
  const oomPatterns = language
    ? ((getLanguageConfig(language) as { oomPatterns?: RegExp[] } | undefined)?.oomPatterns ?? [])
    : [];
  ...
  if (isConfirmedMLE) {
    return {
      ...
      status: "memory_limit_exceeded" as const,
      statusDescription: result.status.description,
      message:
        "Your program used more memory than the allowed limit. Consider reducing memory usage.",
      time: result.time,
      memory: result.memory,
    };
  }
