...
import { FindRelevantPastMessagesInput, RelevantPastMessage } from "./types.js";

export const findRelevantPastMessages = async (
  input: FindRelevantPastMessagesInput,
): Promise<RelevantPastMessage[]> => {
  const {
    chatId,
    activePath, 
    currentEmbedding,
  ...
  try {
    ...
    const mappedResults: RelevantPastMessage[] = results.map((row) => ({
    ...
    logger.info("Semantic backtracking: past messages retrieved", {
      ...
      similarities: mappedResults.map((r) =>
        parseFloat(r.similarity.toFixed(3)),
      ),
    });
