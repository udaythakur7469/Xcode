...
import {
  FetchConversationSummaryResult,
...
const buildIncrementalSummaryPrompt = (
...
const buildInitialSummaryPrompt = (
...
export const fetchConversationSummary = async (
  chatId: string,
  leafMessageId: string, // NEW: current tip of activePath
): Promise<FetchConversationSummaryResult> => {
  try {
...
export const shouldUpdateSummary = async (
  chatId: string,
  activePath: string[], // NEW
  leafMessageId: string, // NEW
): Promise<boolean> => {
  try {
    const record = await prisma.conversationSummary.findUnique({
      where: {
...
export const updateConversationSummary = async (
  ...
  try {
    ...
    if (!updateRequired) {
      logger.info(
        "Conversation summary update skipped — threshold not reached",
        {
          chatId,
          leafMessageId,
    ...
    const lastSummarizedIdx = lastSummarizedId
      ...
      : -1;
    const idsToSummarize =
      lastSummarizedIdx >= 0
        ? activePath.slice(lastSummarizedIdx + 1)
        : activePath;

    const batchIds = idsToSummarize.slice(
      -MAX_MESSAGES_PER_SUMMARIZATION_BATCH,
    );
    ...
    if (orderedMessages.length === 0) {
      logger.info(
        "Conversation summary update skipped — no new messages on branch",
        {
          chatId,
          leafMessageId,
        },
    ...
    const newSummary = await generateSummaryWithLLM(
...
// ─────────────────────────────────────────────────────────────────────────────
export const archiveAndResetSummaryOnTopicChange = async (
  chatId: string,
  leafMessageId: string, // NEW
): Promise<void> => {
  try {
...
// ─────────────────────────────────────────────────────────────────────────────
export const inheritSummaryForBranch = async (
  chatId: string,
  branchParentMessageId: string | null,
  newLeafMessageId: string,
): Promise<void> => {
  if (!branchParentMessageId) return;
  ...
  try {
    ...
    if (!parentSummary) {
      logger.info(
        `[inheritSummaryForBranch] No parent summary found for chat ${chatId} — new branch starts without summary`,
      );
      return;
    }
    ...
    logger.info(
      `[inheritSummaryForBranch] Summary inherited for new leaf ${newLeafMessageId}`,
      { chatId },
    );
  } catch (error: any) {
