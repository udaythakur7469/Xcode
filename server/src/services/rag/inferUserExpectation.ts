import {
  ExpectedResource,
  InferUserExpectationInput,
  UserExpectationResult,
} from "./types.js";
...
const ruleBasedExpectationDetection = (message: string): boolean => {
...
const inferResourceType = (message: string): ExpectedResource => {
  const normalizedMessage = message.toLowerCase().trim();
...
export const userExpectationChecker = ({
  userMessage,
  normalizedQuery,
  providedCode,
}: InferUserExpectationInput): UserExpectationResult => {
  ...
  if (!providedCode) {
    ...
    console.log(
      "[userExpectationChecker] Blocking - no code provided:",
      result,
    );
    return result;
  }
...
const generateClarificationMessage = (resource: ExpectedResource): string => {
  const messages: Record<ExpectedResource, string> = {
    SUBMISSION:
      "I don't have access to your submitted solution yet. Please paste your code here so I can analyze it accurately.",
    EDITOR:
      "I can't see your editor content. Please copy and paste your code here so I can help you.",
    HIDDEN_STATE:
      "I don't have access to your previous code. Please share it here so I can provide accurate feedback.",
  };
