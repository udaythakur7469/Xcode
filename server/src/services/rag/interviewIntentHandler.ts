...
// Sub-categories so the answer prompt can be more targeted
export type InterviewSubIntent =
  | "PREP_TIPS"          // how to prepare, what to study
  | "QUESTION_PATTERNS"  // what questions get asked for role/stack/level
  | "FEEDBACK_INTERPRET" // what does my score / verdict / category mean
  | "PLATFORM_HOWTO"     // how to use the interview feature on this platform
  | "GENERAL";           // anything else interview-related

export interface InterviewIntentResult {
  isInterviewQuery: boolean;
  subIntent: InterviewSubIntent;
  confidence: number; // 0-1
  extractedRole?: string;    // "frontend engineer"
  extractedStack?: string[]; // ["React", "TypeScript"]
  extractedLevel?: string;   // "senior"
  extractedVerdict?: string; // "RECOMMENDED"
}
...
function keywordFastPath(message: string): boolean {
...
export async function detectInterviewIntent(
  userMessage: string,
): Promise<InterviewIntentResult> {
  const fallback: InterviewIntentResult = {
