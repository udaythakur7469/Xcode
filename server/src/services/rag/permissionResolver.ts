...
import { EditorialAccessTier, HintLevel, PermissionContext, RefusalLevel, SolutionIntent, SolutionPermissionMode } from "./types.js";

export async function getOrCreateHelpState(userId: number, problemId: number) {
...
export async function incrementSolutionAttempt(
  userId: number,
  problemId: number,
): Promise<number> {
...
export async function updateHintLevel(
  userId: number,
  problemId: number,
): Promise<HintLevel> {
  const helpState = await getOrCreateHelpState(userId, problemId);
...
export function computeRefusalLevel(
  attemptCount: number,
  confidenceScore: number,
): RefusalLevel {
  if (attemptCount <= 1) return RefusalLevel.SOFT;
...
export async function updateRefusalLevel(
  userId: number,
  problemId: number,
...
export function getAllowedHintLevel(
...
export function computeSolutionPermissionMode(
...
export function getEditorialAccessTier(
  hasSolved: boolean,
): EditorialAccessTier {
  if (!hasSolved) {
...
export function resolvePermissions(
  ...
  const solutionPermissionMode = computeSolutionPermissionMode(
    intent,
    hasSolved,
  );
  const refusalLevel = computeRefusalLevel(attemptCount, confidenceScore);
  const maxHintLevel = getAllowedHintLevel(
    solutionPermissionMode,
    refusalLevel,
    currentHintLevel,
  );
...
export async function resetHelpStateOnSolve(
  userId: number,
  problemId: number,
): Promise<void> {
