...
import type { PersonalDataResult } from "./personalDataFetcher.js";
import type { ContentDataResult } from "./contentQueryFetcher.js";
...
export interface GeneralAssemblerInput {
  intent: GeneralTopLevelIntent;
  userMessage: string;
  dbResult?: PersonalDataResult | ContentDataResult | null;
  pineconeContext?: string; // for PLATFORM_FEATURE
  userName?: string;
}

export interface GeneralAssemblerOutput {
  response: string;
  shortCircuit: boolean; // true = pipeline fully handled, return immediately
}
...
export async function assembleGeneralResponse(
  input: GeneralAssemblerInput
): Promise<GeneralAssemblerOutput> {
    ...
    switch (intent) {
      case "MY_SUBMISSIONS": {
        ...
        const lines = subs.slice(0, 8).map((s) =>
          `- ${s.problem?.title ?? "?"}: ${s.status} | ${s.language} | ${s.runtime}ms | ${s.testCasesPassed}/${s.totalTestCases} tests | ${new Date(s.createdAt).toLocaleDateString()}`
        );
        return lines.join("\n") || "No submissions.";
      }
      ...
      case "MY_STICKY_NOTES": {
        ...
        const lines = notes.map((n) =>
          `- "${n.title ?? "Untitled"}": ${(n.content ?? "").slice(0, 120)}${(n.content?.length ?? 0) > 120 ? "..." : ""} (updated ${new Date(n.updatedAt).toLocaleDateString()})`
        );
        return lines.join("\n") || "No notes.";
      }
      ...
      case "MY_POSTS": {
        ...
        const lines = posts.map((p) =>
          `- "${p.title}" on ${p.problem?.title ?? "?"} | ${p._count?.comments ?? 0} comments | ${p.isDraftPost ? "draft" : "published"}`
        );
        return lines.join("\n") || "No posts.";
      }
      ...
      case "MY_COMMENTS": {
        ...
        const lines = comments.map((c) =>
          `- On "${c.post?.title ?? "?"}": "${(c.content ?? "").slice(0, 100)}..."`
        );
        return lines.join("\n") || "No comments.";
      }
      ...
      case "MY_PROFILE": {
        ...
        return `Name: ${u.name ?? "not set"}\nEmail: ${u.email}\nBio: ${u.description ?? "not set"}\nInstitution: ${u.institution ?? "not set"}\nLinks: ${u.links?.map((l: any) => `${l.key}: ${l.value}`).join(", ") || "none"}`;
      }

      case "PROBLEM_POSTS": {
        const posts = (data.posts as any[]) ?? [];
        const lines = posts.map((p) =>
          `- "${p.title}" by ${p.author?.name ?? "?"} | ${p._count?.comments ?? 0} comments | ${new Date(p.createdAt).toLocaleDateString()}`
        );
        return lines.join("\n") || "No posts.";
      }

      case "TEST_CASES": {
        const examples = (data.examples as any[]) ?? [];
        const lines = examples.map((e, i) =>
          `Example ${i + 1}: Input: ${e.input} → Output: ${e.output}${e.explanation ? ` (${e.explanation})` : ""}`
        );
        return lines.join("\n") || "No examples.";
      }
