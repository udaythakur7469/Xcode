...
const REFUSAL_TONE_MAP: Record<RefusalLevel, string> = {
  [RefusalLevel.SOFT]:
    "You may gently redirect the user toward self-discovery without giving the answer.",
  [RefusalLevel.FIRM]:
    "You must firmly decline to provide any solution or near-complete code. Redirect with encouragement.",
  [RefusalLevel.STRICT]:
    "You must strictly refuse to provide any solution, partial solution, or code hint. Only ask guiding questions.",
};

const HINT_LEVEL_MAP: Record<HintLevel, string> = {
  [HintLevel.NONE]: "Do not provide any hints.",
  [HintLevel.CONCEPT]:
    "You may provide a conceptual hint about the general problem-solving strategy only.",
  [HintLevel.DATA_STRUCTURE]:
    "You may suggest the relevant data structure or algorithm category.",
  [HintLevel.EDGE_CASES]:
    "You may discuss edge cases and boundary conditions without revealing the implementation.",
};

const EDITORIAL_ACCESS_MAP: Record<EditorialAccessTier, string> = {
  [EditorialAccessTier.NONE]:
    "Do not reference or reveal any editorial content.",
  [EditorialAccessTier.HINTS_ONLY]:
    "You may reference hints from the editorial but not the full solution.",
  [EditorialAccessTier.FULL]:
    "You may reference the full editorial content to assist the user.",
};
...
const REGENERATE_DEPTH_INSTRUCTION =
  "The user has explicitly requested a new response. " +
...
const buildPermissionDirectives = (input: AnswerGenerationInput): string => {
  ...
  if (
    permissions.solutionPermissionMode === SolutionPermissionMode.HINTS_ONLY
  ) {
  ...
  if (
    ...
    directives.push(
      "You may provide a complete, correct solution with full explanation.",
    );
  }
...
const buildPastMessagesSection = (
  relevantPastMessages: RelevantPastMessage[],
): string => {
  if (relevantPastMessages.length === 0) return "";

  const snippets = relevantPastMessages.map((msg, index) => {
    const snippet =
      msg.content.length > MAX_PAST_MESSAGE_SNIPPET_LENGTH
...
const buildRecentMessagesSection = (recentMessages: string[]): string => {
...
const buildDocumentContext = (docs: RetrievedKnowledge[]): string => {
  if (docs.length === 0) {
  ...
  const snippets = docs
    ...
    .map((doc, index) => {
      const snippet =
        doc.metadata.assistantResponse.length > MAX_DOCUMENT_SNIPPET_LENGTH
...
function resolveModel(aiModel: string) {
  const normalized = aiModel.toLowerCase().trim();

  if (normalized === "gemini") return google(MODEL_GEMINI);
  if (normalized === "claude") return anthropic(MODEL_CLAUDE);
  if (normalized === "chatgpt") return openai(MODEL_OPENAI);
...
const buildSystemPrompt = (input: AnswerGenerationInput): string => {
...
const buildUserPrompt = (input: AnswerGenerationInput): string => {
  const { normalizedMessage, filteredDocuments, assembledContext } = input;

  const pastMessagesSection = buildPastMessagesSection(
    assembledContext.relevantPastMessages,
  );
  const recentMessagesSection = buildRecentMessagesSection(
    assembledContext.recentMessages,
  );
...
export const answerGeneration = async (
