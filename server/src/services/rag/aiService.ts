...
export const generateAIResponse = async (
  ...
  try {
    ...
    console.log(
      "[generateAIResponse] User expectation check:",
      expectationCheck,
    );
    ...
    if (expectationCheck.shouldBlock) {
      console.log(
        "[generateAIResponse] Blocked - user expects implicit access:",
        expectationCheck.expectedResource,
      );
  ...
  try {
    ...
    if (processorResult.shouldBlock) {
      console.log(
        "[generateAIResponse] Request blocked by solution intent gate",
      );
    ...
    console.log(
      "[generateAIResponse] Solution processor passed with permissions:",
      permissions,
    );
    ...
    try {
      ...
      if (artifactCheck.blockGeneration) {
        console.log(
          "[generateAIResponse] Blocked - missing artifacts:",
          artifactCheck.missingArtifacts,
        );
  ...
  const embeddedNormalizedMessage: number[] =
    await generateEmbedding(normalizedMessage);

  // Step 6: Search Pinecone for relevant past knowledge
  const retrievedKnowledge: RetrievedKnowledge[] = await searchAiChatKnowledge(
