...
} from "./conversationSummaryManager.js";
...
import {
  runAutoRetrievalRetry,
  AUTO_RETRY_CONFIDENCE_THRESHOLD,
} from "./retrievalRetry.js";
...
const BYPASS_PERMISSIONS: PermissionContext = {
...
const BYPASS_FALLBACKS: Record<string, string> = {
  GREETING:
    "I'm here and ready to help! Feel free to paste your code or describe what you're working on.",
...
export const generateAIResponse = async (
  ...
  const { regenerateCount, aiModelChanged, lastRetrievalConfidence } =
    await getOrUpdateRegenerateState(
      chatId,
      userMessageId,
      regenerate,
      aiModel,
  ...
  if (intentRoute.shouldBypassRAG) {
    ...
    let bypassRawAnswer =
      BYPASS_FALLBACKS[intentRoute.detectedPattern ?? "GREETING"] ??
...
Context: problem "${problemTitle ?? "none"}", pattern "${intentRoute.detectedPattern}".`,
  ...
  const activeMessage =
    intentRoute.isMixedIntent && intentRoute.taskSegment !== null
  ...
  const isInterviewQuery =
    interviewIntent.isInterviewQuery && interviewIntent.confidence > 0.4;
  ...
  const isGeneralQuery =
    generalIntent.isGeneralQuery && generalIntent.confidence > 0.45;
    ...
    if (generalIntent.intent === "PLATFORM_FEATURE") {
      try {
        ...
        pineconeContext = platformDocs
          .map((d: any) => d.metadata?.assistantResponse ?? "")
          .join("\n\n");
      } catch (err) {
  ...
  if (!isInterviewQuery && !isGeneralQuery) {
    try {
      ...
      if (expectationCheck.shouldBlock) {
        ...
        return (
          expectationCheck.clarificationMessage ||
  ...
  let permissions: PermissionContext | null = null;
  let detectedIntent = null;
  let intentConfidence = 0;

  if (!isInterviewQuery && !isGeneralQuery) {
    try {
      const processorResult = await solutionIntentProcessor(
        userId,
        problemTitle,
        activeMessage,
        probeUserMessages,
        ...
        if (problemId) {
          try {
            const denialCtx = await buildDenialContext({
              ...
              refusalLevel:
                processorResult.permissions?.refusalLevel ?? RefusalLevel.SOFT,
          ...
          if (problemId) {
            try {
              const denialCtx = await buildDenialContext({
                ...
                refusalLevel:
                  processorResult.permissions?.refusalLevel ??
                  RefusalLevel.SOFT,
                solutionAttemptCount: 0,
              });
              const denialResult = await generateDenialResponse(denialCtx);
  ...
  if (topicShiftResult.strategy === "RESET") {
    archiveAndResetSummaryOnTopicChange(chatId, currentLeaf).catch(
      (
        err, // CHANGED: added currentLeaf
      ) =>
        logger.error("Phase 6: failed to archive summary on topic reset", {
          err,
          chatId,
        }),
    );
  ...
  const originalEmbedding: number[] =
    await generateEmbedding(normalizedMessage);
  ...
  let tier2Strategy: string | null = null;
  ...
  if (regenerate) {
    ...
    if (regenStrategy.retrievalOptions.expandedQuery !== null) {
      activeEmbedding = await generateEmbedding(
        regenStrategy.retrievalOptions.expandedQuery,
      );
    }
  ...
  let relevantPastMessages: Awaited<
    ReturnType<typeof findRelevantPastMessages>
  > = [];
  ...
  const retrievedKnowledge: RetrievedKnowledge[] =
    await searchAiChatKnowledgeWithNamespace(
      activeEmbedding,
      activeTopK,
      chatId,
      pineconeNamespace,
      pineconeNamespace ? undefined : userId,
    );
  ...
  const initialFilterResult: FilterResult = await filterDocuments(
    retrievedKnowledge,
    activeFilterThreshold,
  );
  ...
  if (
    initialFilterResult.retrievalConfidence < AUTO_RETRY_CONFIDENCE_THRESHOLD
  ) {
  ...
  storeMessageEmbedding({
    ...
    logger.error("Phase 18A: failed to store message embedding", {
      err,
      chatId,
    }),
  );
  ...
  shouldUpdateSummary(chatId, activePath, currentLeaf) 
    ...
    .catch((err) =>
      logger.error("Phase 18B: failed to update conversation summary", {
        err,
        chatId,
      }),
    );

  persistLastRetrievalConfidence(
    chatId,
    userMessageId,
    filteredDocuments.retrievalConfidence,
  ).catch((err) =>
    logger.error("Phase 18C: failed to persist retrieval confidence", {
      err,
      chatId,
      userMessageId,
    }),
...
const persistLastRetrievalConfidence = async (
  chatId: string,
...
async function searchAiChatKnowledgeWithNamespace(
  embedding: number[],
  topK: number,
  chatId: string,
  namespace?: string,
...
function storeAiChatKnowledgeBackground(input: {
  ...
  import("./pineconeService.js")
    .then(({ storeAiChatKnowledge }) =>
      storeAiChatKnowledge({
    ...
    .catch((err) =>
      logger.error("Phase 18D: failed to store in Pinecone", {
        err,
        chatId: input.chatId,
      }),
    );
