...
import {
  MixedIntentSplitterInput,
  MixedIntentSplitterResult,
} from "./types.js";
...
const containsTaskSignal = (text: string): boolean => {
  const lower = text.toLowerCase();
  return TASK_SIGNAL_WORDS.some((signal) => lower.includes(signal));
};

export const mixedIntentSplitter = async (
  input: MixedIntentSplitterInput,
): Promise<MixedIntentSplitterResult> => {
