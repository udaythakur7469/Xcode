...
const NEUTRAL_INTENSITY: StyleIntensity = 2;

// First-denial intensity — new users / new problems start at ENCOURAGING.
const INITIAL_INTENSITY: StyleIntensity = 3;
...
export const mapIntensityToStyle = (intensity: StyleIntensity): RefusalStyle => {
  switch (intensity) {
...
export const computeNextStyleState = (input: StyleDecayInput): StyleState => {
  const {
    previousState,
    repeatedAttempt,
    confidenceLow,
  ...
  let nextIntensity: StyleIntensity = previousState.intensity;

  if (repeatedAttempt || confidenceLow) {
    nextIntensity = Math.max(
      (previousState.intensity - 1) as StyleIntensity,
      0,
    ) as StyleIntensity;
  }
