...
import {
  NonTaskPattern,
  SolutionIntent,
  UnifiedIntentRouterInput,
  UnifiedIntentRouterResult,
} from "./types.js";
...
const detectInterviewPrepQuery = (message: string): boolean =>
  INTERVIEW_PREP_PATTERNS.some((p) => p.test(message));

const detectNonTaskPattern = (
  message: string,
): NonTaskPattern | null => {
  ...
  for (const pattern of PING_PATTERNS) {
    if (pattern.test(trimmed)) return "PING";
  }

  return null;
};

const hasMixedIntentPrefix = (message: string): boolean =>
  MIXED_INTENT_PREFIX_PATTERNS.some((p) => p.test(message));

export const unifiedIntentRouter = async (
  input: UnifiedIntentRouterInput,
): Promise<UnifiedIntentRouterResult> => {
  ...
  if (hasMixedIntentPrefix(trimmed)) {
    ...
    if (splitResult.isMixed && splitResult.taskSegment !== null) {
      ...
      const isInterviewMixed = detectInterviewPrepQuery(
        splitResult.taskSegment ?? trimmed,
      );
      return {
