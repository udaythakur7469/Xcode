...
const ruleBasedDetection = (message: string): SolutionIntent | null => {
...
const llmBasedRuleDetection = async ({
  userMessage,
  previousUserMessages,
  problemTitle,
  difficulty,
  hasSolved,
}: IntentDetectionInput): Promise<IntentDetectionResult> => {
  const prompt = solutionIntentClassificationPromptBuilder(
    userMessage,
    previousUserMessages,
    hasSolved,
    problemTitle,
  ...
  const intent =
    validIntents.find((i) => text.includes(i)) || SolutionIntent.FULL_SOLUTION;
...
export const detectSolutionIntent = async ({
  userMessage,
  previousUserMessages,
  problemTitle,
  difficulty,
  ...
  console.log(
    "[detectSolutionIntent] Starting intent detection for message:",
  ...
  if (ruleResult) {
    console.log(
      "[detectSolutionIntent] Rule-based detection matched:",
      ruleResult,
    );
    return {
      intent: ruleResult,
