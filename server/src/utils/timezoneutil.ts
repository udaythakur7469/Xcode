...
export function getLocalDayBoundsUtc(
...
): { startUtc: Date; endUtc: Date; timeZoneUsed: string } {
  ...
  const localMidnightLabeledAsUtc = Date.UTC(
  ...
  const endUtc = new Date(
    localMidnightLabeledAsUtc + 86_400_000 - 1 - offsetMs,
  );
...
export function localDayDiff(
  ...
  const localDayA = Math.floor((a.getTime() + offsetA) / 86_400_000);
  const localDayB = Math.floor((b.getTime() + offsetB) / 86_400_000);
